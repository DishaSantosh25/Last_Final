...
# Replace this with your actual model definition if different.
class MyModel(nn.Module):
    def __init__(self):
        super(MyModel, self).__init__()
        # Example architecture: one convolution, pooling, and a fully-connected layer.
...
# Model loader using state dictionary
@st.cache_resource
def load_model():
            ...
            if st.button("Analyze Image", key="analyze_camera"):
                with st.spinner("📊 Analyzing your wheat leaf..."):
                    ...
                    if class_names[result_index] == "Healthy":
                        st.markdown(
                            '<div class="disease-result disease-healthy">✨ Your wheat plant is healthy!</div>',
                            unsafe_allow_html=True
                        )
                    else:
