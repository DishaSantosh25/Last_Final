...
# IMPORTANT: Replace this with the exact architecture used during training.
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(16, 5)  # Assume 5 classes: Brown_rust, Healthy, Loose_Smut, Yellow_rust, septoria
...
# Load and encode the wheat image for the header banner
def get_wheat_image():
    try:
...
def load_model():
    ...
    model = MyModel()
    # Load the state dictionary with strict=False to bypass missing key errors
    state_dict = torch.load("./wheat_disease_model.pth", map_location=device)
